...
chrome.tabs.onUpdated.addListener(function callback(tabid,info,tab)
{
    ...
    if(info.status==="loading" && tab.url.indexOf("hackerrank.com")!=-1)
	{
       ...
       console.log("inject");
        chrome.tabs.executeScript(tabid, {file:"inject_hackerrank.js",allFrames:true}, null);
...
chrome.tabs.onUpdated.addListener(function callback(tabid,info,tab)
{
 ...
 console.log(info.status);
	if(tab.url.indexOf("codechef.com")!=-1)
	{
        //inject the chamcha script
       console.log("inject");
        chrome.tabs.executeScript(tabid, {file:"inject_codechef.js",allFrames:true,runAt:"document_end"}, null);
...
chrome.tabs.query({url:"*://*.hackerearth.com/*"}, function callback(tabs)
{
    chrome.tabs.query({url:"*://*.codechef.com/*"}, function callback(tabs1)
	{
			chrome.tabs.query({url:"*://*.hackerrank.com/*"}, function callback(tabs2)
			{
			//add tabs1 to tabs
			...
			chrome.tabs.update(tabid,{active:true},null); //make tab come into focus
